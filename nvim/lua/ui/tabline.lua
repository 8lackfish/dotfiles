...
local function bufline()
  ...
  Tabline option automatically truncates the beginning of the input
  string when its width exceeds the window width.

  The focused buffer item should always stay in a non-truncated scope.
  That should be the minimum requirement.

  Simplest way to meet minimal requirement based on automatic-truncation
  mechanism is to always return a string that begins or ends at the
  focused item whenever input string is detected to exceed window width.
  But, this approach limits how intuitive it feels to navigate focus. So,
  still based on the automatic-truncation mechanism, a better way is to
  always return a string that spans outward from the position of the
  focused item in both directions in the buffer list whenever input
  string is detected to exceed window width.

  Input string consists of three main sections: left | focused | right,
  with two cases: basic case and overflow case.

  The overflow case occurs when:
  left_width + focused_width + right_width > nvim_win_width

  Then:
  focused_width + right_width > nvim_win_width - left_width
  This is true for any width of the left section
...
--- Setup bufferline on the tabline.
